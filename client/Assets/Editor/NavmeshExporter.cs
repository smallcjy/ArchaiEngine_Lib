...
public class NavmeshExporter : Editor
{
    ...
    [MenuItem("ExportScene/ExportNavmesh")]
    static void Export(MenuCommand command)
    {
        string exportPath = GetSavePath(false, null);
        ...
        {
            writer.WriteLine("v " + -navMeshData.vertices[i].x + " " + navMeshData.vertices[i].y + " " + navMeshData.vertices[i].z);
        }

        writer.WriteLine("g pPlane1");

        for (int i = 0; i < navMeshData.indices.Length;)
        {
            writer.WriteLine("f " + (navMeshData.indices[i + 1] + 1) + " " + (navMeshData.indices[i + 2] + 1) + " " + (navMeshData.indices[i] + 1));
            i = i + 3;
        }
        ...
        Debug.Log($"NavMesh exported to: {exportPath}");
    }
